...
const getNotesById = asyncHandler(async (req, res) => {
  const { id } = req.params;
  const note = await Note.findOne({ noteId: id, userId: req.user.userId });
  if (!note) {
    res.status(constants.NOT_FOUND);
...
const updateNotes = asyncHandler(async (req, res) => {
  ...
  const updatedNote = await Note.findOneAndUpdate(
    ...
    { noteId: id, userId: req.user.userId },
...
const deleteNotes = asyncHandler(async (req, res) => {
  ...
  }); // here also not using findByIdAndDelete for same reason as above (since it takes mongodb _id not out custom noteId)
  if (!deletedNote) {
    res.status(constants.NOT_FOUND);
